# XMC&trade; MCU: CCU4 timer concatenation

This code example demonstrates the concatenation or linking feature of the capture and compare unit 4 (CCU4) slices available in XMC&trade; MCU.

The code example uses three CCU4 slices of the XMC&trade; MCU:

   1. One slice configured as timer. It generates a time base of 1 us. It will be used to count up the timer of the other two.
...
- [ModusToolbox&trade; software](https://www.infineon.com/products/modustoolbox) v3.0
...
The following example will clone the "[CCU4 timer concatenation](https://github.com/Infineon/mtb-example-xmc-ccu4-timer-concatenation)" application with the desired name "CCU4_Timer_Concatenation" configured for the *KIT_XMC47_RELAX_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC47_RELAX_V1 --app-id mtb-example-xmc-ccu4-timer-concatenation --user-app-name CCU4_Timer_Concatenation --target-dir "C:/mtb_projects"
   ```
...
The application uses the following CCU4 slices configured using the CCU4 personality:

   1. TIMER_0 configured as a timer, providing a 1-us time base.

   2. COUNTER_H and COUNTER_L configured as counter. These two slices are linked together forming a 32-bit counter. They will increase their timer value based on the 1-us time base, using the falling edge of the slice comparison status signal (CC4x.STy) from TIMER_0. Currently, the CCU4 personality offers only the  external pins for the event input signal. To overcome this limitation, the XMCLib peripheral driver for CCU4 function, `XMC_CCU4_SLICE_SetInput`, is used.

   **Figure 1. COUNTER_L configuration**

   ![](images/counter_l_personality.png)

The TIMER_0, COUNTER_H, and COUNTER_L resources are not enabled in the default board configuration provided by the BSP. This example overrides the `BSP_DESIGN_MODUS` component provided with the BSP to enable and configure  CYBSP_DEBUG_UART. See the "Overriding the BSP configuration files" section of the [ModusToolbox&trade; software user guide](https://www.infineon.com/ModusToolboxUserGuide): *{ModusToolbox&trade; install directory}/ide_{version}/docs/mtb_user_guide.pdf*

The main loop will toggle CYBSP_USER_LED if the elapsed time since the reference was captured exceeds `USER_LED_TOGGLE_PERIOD_US`.
...
Document title: *CE231960* - *XMC&trade; MCU: CCU4 timer concatenation*
